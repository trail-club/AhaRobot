...
**可動量が極端に狭いと表示されたら**: STS3215 の位置指令は 0-4095 の絶対値で、
範囲外は出せない。エンコーダ原点付近に止まっているサーボがあると、その 1 個だけ
動けずに残りと押し合って関節が暴れる。起動時の表で「★原点近くに張り付いています」と
出たサーボは、手で動かして原点から離すか、オフセットレジスタを書き直す必要がある
(ファームの `setupTorque(128)` が本来やること)。

```
関節            +方向     -方向   制限しているサーボ
joint0       1000       0   -側: ID5  ★原点近くに張り付いています
```
